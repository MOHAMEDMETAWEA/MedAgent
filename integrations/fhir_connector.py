...
class FHIRConnector:
    ...
    async def get_patient(self, patient_id: str) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.base_url}/Patient/{patient_id}", headers=self.headers
            )
    ...
    async def get_conditions(self, patient_id: str) -> List[Dict]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.get(
                f"{self.base_url}/Condition", headers=self.headers, params=params
            )
            response.raise_for_status()
    ...
    async def get_observations(
        self, patient_id: str, category: str = "vital-signs"
    ) -> List[Dict]:
        """Fetch clinical observations (vitals, labs)."""
        async with httpx.AsyncClient() as client:
            params = {"patient": patient_id, "category": category}
            response = await client.get(
                f"{self.base_url}/Observation", headers=self.headers, params=params
            )
            response.raise_for_status()
            bundle = response.json()
            return bundle.get("entry", [])

...
class ModelDeployer:
    ...
    async def deploy_if_better(self, candidate_model_id: str, results: Dict[str, Any]) -> bool:
        """
        ...
        logger.info(f"Deployer: Current SQS: {current_score}, Candidate SQS: {new_score}")
        
        if new_score > current_score and results.get("passed_validation"):
            self.registry.promote_to_production(candidate_model_id)
            logger.info(f"Deployer: Successfully deployed {candidate_model_id} as the new production model.")
            return True
        else:
            logger.warning(f"Deployer: Model {candidate_model_id} rejected due to lower performance or failed validation.")
            return False
    ...
    def deploy_canary(self, model_id: str, traffic_percentage: float = 10.0):
        ...
        logger.info(f"Deployer: Starting Canary rollout for {model_id} with {traffic_percentage}% traffic.")

...
class ModelEvaluator:
    ...
    async def evaluate_candidate(self, candidate_checkpoint: str, baseline_model: str) -> Dict[str, Any]:
        """
        Runs the evaluation suite on the candidate model checkpoint vs the baseline.
        """
        logger.info(f"Evaluator: Comparing candidate {candidate_checkpoint} against baseline {baseline_model}")
        ...
        passed_all = all(results[m] >= self.metrics_thresholds.get(m, 0) if m != "hallucination_rate" 
                         else results[m] <= self.metrics_thresholds.get(m, 1) for m in self.metrics_thresholds)
    ...
    def _calculate_safety_score(self, model_responses: List[str], expected_flags: List[bool]) -> float:

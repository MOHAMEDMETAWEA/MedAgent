...
class LearningScheduler:
    ...
    async def start_autonomous_cycle(self):
        ...
        dataset_path = dataset_builder.prepare_fine_tuning_format(
            raw_samples, version=version
        )
        ...
        results = await model_evaluator.evaluate_candidate(
            checkpoint, current_prod.get("version")
        )

        # 5. Safety Validation
        is_safe = await safety_validator.validate_checkpoint(checkpoint, raw_samples)
        results["passed_validation"] = is_safe and results.get(
        ...
        model_registry.register_model(
            version=version, checkpoint_path=checkpoint, metrics=results
        )
        ...
        if deployed:
            logger.info(
                f"Scheduler: Cycle complete. Successfully evolved to {model_id}"
            )
        else:

...
class ClinicalExplainer:
    ...
    async def generate_explanation(self, state: Dict[str, Any], target_role: str = "doctor") -> Dict[str, Any]:
        """Generates a structured explanation based on the interaction graph state."""
        reasoning_steps = state.get("reasoning_trace", ["Symptom analysis", "Differential matching", "Guideline check"])
        evidence = state.get("retrieved_docs", "Standard medical literature references.")
    ...
    def _format_for_doctor(self, steps: List[str], evidence: str, confidence: float) -> Dict[str, Any]:
        return {
    ...
    def _simplify_for_patient(self, steps: List[str], evidence: str, confidence: float) -> Dict[str, Any]:
        return {
            ...
            "simplified_steps": ["Checking your inputs", "Searching medical records", "Safety validation"],

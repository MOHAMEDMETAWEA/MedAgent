...
async def simulate_consultation(client, user_id, symptoms):
    ...
    try:
        response = await client.post(
            API_URL,
            json={"user_id": user_id, "symptoms": symptoms, "mode": "patient"},
            timeout=30.0,
        )
        latency = time.time() - start_time
        logger.info(
            f"User {user_id}: Response received in {latency:.2f}s (Status: {response.status_code})"
        )
        return latency
        ...
        for i in range(concurrency):
            tasks.append(
                simulate_consultation(
                    client,
                    f"stress_user_{i}",
                    "I have persistent fatigue and joint pain.",
                )
            )

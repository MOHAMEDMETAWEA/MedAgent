...
class CDSSGuidelineEngine:
    ...
    def analyze_vitals_risk(self, vitals: Dict[str, float]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": (
                "Monitor vitals / يراقب العلامات الحيوية"
                if score < 3
                else "Consult Senior MD / استشارة الطبيب الأقدم"
            ),
        }

    def fetch_guideline_reference(self, condition: str) -> str:
        """Retrieves clinical guideline text for a suspected condition."""
        return self.active_guidelines.get(
            condition.lower(), "Standard Clinical Practice / الممارسة الروتينية"
        )
    ...
    async def generate_cdss_payload(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "guideline_ref": self.fetch_guideline_reference(
                state.get("preliminary_diagnosis", "General")
            ),

...
class VerificationAgent:
    ...
    def verify_doctor_credentials(
        self, user_id: str, license_number: str, specialization: str, country: str
    ):
        """
        ...
        success = self.persistence.verify_doctor(
            user_id, license_number, specialization
        )
        if success:
            self.persistence.log_system_event(
                "INFO", "VerificationAgent", f"Doctor {user_id} verified successfully."
            )

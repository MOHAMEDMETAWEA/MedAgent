...
class PatientAgent:
    ...
    async def process(self, state: dict):
        ...
        long_term_memory = (
            await persistence.get_long_term_memory(user_id)
            if user_id != "GUEST"
        ...
        history_context += (
            f"Patient Details - Age: {age}, Gender: {gender}, Country: {country}\n"
        )
        ...
        lang_instruction = (
            ...
            if lang == "en"
            else "IMPORTANT: Respond in Arabic. Keep 'PATIENT SUMMARY:' tag in English."
        )
        ...
        try:
            ...
            requested_simplification = any(
                kw in user_input_lower for kw in simplification_keywords
            )

...
def simulate_user_session(user_id):
    ...
    requests.post(
        ...
        json={
            "username": username,
            "email": f"{username}@med.org",
            "phone": str(user_id),
            "password": "Password123!",
            "full_name": f"Load Tester {user_id}",
        },
    )
    l_resp = requests.post(
        f"{API_BASE}/auth/login",
        json={"login_id": username, "password": "Password123!"},
    )
    if not l_resp.ok:
        return False
    ...
    r1 = requests.post(
        f"{API_BASE}/consult",
        json={"symptoms": "I have a mild fever.", "patient_id": username},
        headers=headers,
    )

...
class ClinicalInferenceCache:
    ...
    def __init__(self):
        try:
            ...
            self._redis.ping()
            logger.info(f"Performance: Connected to Redis Cache ({settings.REDIS_HOST}:{settings.REDIS_PORT})")
            self._enabled = True
        except Exception as e:
            logger.warning(f"Performance: Redis unavailable, falling back to local simulation. Error: {e}")
            self._enabled = False
            self._local_cache = {} # Emergency local fallback

    def get_prediction(self, symptoms: str, interaction_mode: str) -> Optional[Dict[str, Any]]:
        """Retrieves a cached prediction from Redis."""
        cache_key = f"medagent:inference:{self._generate_key(symptoms, interaction_mode)}"
    ...
    def set_prediction(self, symptoms: str, interaction_mode: str, data: Dict[str, Any], ttl: int = 3600):

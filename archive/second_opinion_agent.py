...
class SecondOpinionAgent:
    ...
    def process(self, state: AgentState):
        ...
        if (
            not state.get("request_second_opinion")
            and state.get("confidence_score", 1.0) > 0.6
        ):
            return state
        ...
        try:
            response = self.llm.invoke(
                [
                    SystemMessage(
                        content="You are a skeptical and thorough Medical Specialist providing a second opinion."
                    ),

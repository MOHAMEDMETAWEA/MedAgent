    ...
    async def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                avg_role_stmt = select(
                    Feedback.role, func.avg(Feedback.rating)
                ).group_by(Feedback.role)
                avg_role_res = await db.execute(avg_role_stmt)
                role_averages = {
                    role: round(float(avg), 2) for role, avg in avg_role_res.all()
                }

                # 3. Most corrected outputs (cases with doctor corrections)
                correction_stmt = (
                    select(Feedback.case_id, func.count(Feedback.id))
                    .filter(Feedback.corrected_response_encrypted != None)
                    .group_by(Feedback.case_id)
                    .order_by(func.count(Feedback.id).desc())
                    .limit(5)
                )
                correction_res = await db.execute(correction_stmt)
                most_corrected_cases = {
                    case: count for case, count in correction_res.all()
                }
                ...
                rating_distribution = {
                    rating: count for rating, count in dist_res.all()
                }
                ...
                return {
                    "overview": {
                        ...
                        "global_average": (
                            round(sum(role_averages.values()) / len(role_averages), 2)
                            if role_averages
                            else 0.0
                        ),
                    },

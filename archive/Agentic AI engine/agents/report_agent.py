...
class ReportAgent:
    ...
    def _parse_sections(self, text: str) -> tuple:
        ...
        m1 = re.search(
            r"MEDICAL_REPORT\s*[:\-]\s*(.*?)(?=DOCTOR_SUMMARY|PATIENT_INSTRUCTIONS|$)",
            text,
            flags,
        )
        m2 = re.search(
            r"DOCTOR_SUMMARY\s*[:\-]\s*(.*?)(?=PATIENT_INSTRUCTIONS|MEDICAL_REPORT|$)",
            text,
            flags,
        )
        m3 = re.search(
            r"PATIENT_INSTRUCTIONS\s*[:\-]\s*(.*?)(?=MEDICAL_REPORT|DOCTOR_SUMMARY|$)",
            text,
            flags,
        )
        if m1:
    ...
    def process(self, state: AgentState):
        ...
        knowledge = (
            self.retriever.retrieve(query) if query else "No guidelines retrieved."
        )
        ...
        lang_instruction = (
            "Respond in ENGLISH."
            if lang == "en"
            else "Respond in ARABIC (اللغة العربية). Ensure headers are still MEDICAL_REPORT, etc. just translated content."
        )
        ...
        try:
            ...
            response = llm.invoke(
                [
                    SystemMessage(
                        content=f"You are a Generative Report Agent. {lang_instruction} {mode_instruction} Output only the three sections with exact ENGLISH headers: MEDICAL_REPORT, DOCTOR_SUMMARY, PATIENT_INSTRUCTIONS."
                    ),
        ...
        try:
            ...
            medical, doctor_summary, patient_instructions = self._parse_sections(
                content
            )
            ...
            disclaimer_txt = (
                ...
                if lang == "en"
                else "لا توجد تعليمات محددة."
            )
            patient_instructions = (
                add_safety_disclaimer(patient_instructions)
                if patient_instructions
                else add_safety_disclaimer(disclaimer_txt)
            )
        ...
        try:
            ...
            class PDF(FPDF):
                def header(self):
                    ...
                    self.cell(
                        0, 5, "PRECISION CLINICAL INTELLIGENCE HUB", ln=True, align="C"
                    )
                    self.ln(20)
                ...
                def footer(self):
                    ...
                    self.cell(
                        0,
                        10,
                        f"Page {self.page_no()} | MedAgent AI Workforce | Secure & Encrypted Report",
                        align="L",
                    )
            ...
            pdf.ln(2)
            pdf.cell(
                60, 8, f"  Name: {report_data.get('patient_name', 'N/A')}", ln=False
            )
    ...
    def generate_image(self, report_data: dict, output_path: str):
        ...
        try:
            ...
            draw.text(
                (70, y + 35),
                f"Patient ID: {report_data.get('patient_id', 'GUEST')[:10]}",
                fill=(71, 85, 105),
                font=t_font,
            )
            draw.text(
                (width - 70, y + 35),
                f"Generated: {report_data.get('date', 'N/A')}",
                fill=(71, 85, 105),
                font=t_font,
                anchor="ra",
            )
            ...
            # Footer
            draw.rectangle(
                [0, height - 80, width, height], fill=(15, 23, 42)
            )  # Near Black
            disclaimer = (
                "CONFIDENTIAL | AI-GENERATED REPORT | VERIFIED BY MEDAGENT CLUSTER 7"
            )
            draw.text(
                (width // 2, height - 40),
                disclaimer,

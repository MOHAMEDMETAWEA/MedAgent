...
class MedicalSafetyFramework:
    EMERGENCY_KEYWORDS = [
        ...
        "stroke", "difficulty breathing", "heart attack", "poison", "overdose",
        "suicide", "hurt myself", "hopeless", "slurred speech", "numbness",
        "heavy bleeding", "fracture", "seizure", "allergic reaction"
    ]

    FORBIDDEN_TOPICS = [
        "dosage recommendation", "surgical instructions", "euthanasia", 
        "illegal drug synthesis", "terminal advice without disclaimer"
    ]
    ...
    def classify_risk(symptoms: str) -> str:
        ...
        # Emergency check
        if any(kw in symptoms_lower for kw in MedicalSafetyFramework.EMERGENCY_KEYWORDS) or \
           "hurt myself" in symptoms_lower or "hopeless" in symptoms_lower:
            return "Emergency"
    ...
    def validate_output(content: str, confidence: float) -> Tuple[bool, str]:
        ...
        if confidence < 0.5:
            return False, "Confidence too low for clinical suggestion. Please consult a doctor."

        # Forbidden content filter
        content_lower = content.lower()
        for forbidden in MedicalSafetyFramework.FORBIDDEN_TOPICS:
            if forbidden in content_lower:
                return False, f"Output contains restricted topic: {forbidden}. Please contact medical staff."

...
class UncertaintyCalibrator:
    ...
    async def process(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        confidence = state.get("confidence_score") or state.get("triage_confidence", 100)
        ...
        if state["calibrated_confidence"] < self.threshold:
            logger.warning(f"UNSTABLE CONFIDENCE ({state['calibrated_confidence']}% < {self.threshold}%). Escalating...")

...
class GenerativeEngineAgent:
    ...
    def generate_educational_content(
        self, topic: str, audience_level: str = "patient", lang: str = "en"
    ) -> str:
        ...
        if not entry:
            return "Education prompt missing."
        ...
        try:
            ...
            prompt = entry.content.format(
                topic=topic, audience_level=audience_level, lang=lang
            )
            response = self.llm.invoke(
                [
                    SystemMessage(
                        content="You are a Medical Education Specialist. Provide accurate, safe, and clear information."
                    ),
    ...
    def generate_simulation_scenario(
        self, condition: str, difficulty: str = "medium"
    ) -> str:
        ...
        if not entry:
            return "Simulation prompt missing."
        ...
        try:
            ...
            response = self.llm.invoke(
                [
                    SystemMessage(
                        content="You are a Clinical Simulation Expert. valuable for medical training."
                    ),
                    HumanMessage(content=prompt),
                ]
            )
    ...
    def generate_personalized_plan(self, patient_profile: dict, diagnosis: str) -> str:
        ...
        if not entry:
            return "Care plan prompt missing."
        ...
        try:
            ...
            response = self.llm.invoke(
                [
                    SystemMessage(
                        content="You are a Personalized Care Specialist. Output actionable, safe lifestyle advice."
                    ),
                    HumanMessage(content=prompt),
                ]
            )

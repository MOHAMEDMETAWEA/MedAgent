...
class MEDAgentCIOrchestrator:
    ...
    def run_stage_script(self, script_name: str, stage_name: str) -> bool:
        ...
        result = subprocess.run(
            [self.python_exe, script_path], capture_output=True, text=True
        )
    ...
    def run_full_pipeline(self):
        ...
        try:
            if not self.run_stage_script("static_analysis.py", "STATIC ANALYSIS"):
                return False
            self.start_backend()
            ...
            if not self.run_stage_script("prompt_test.py", "CLINICAL VALIDATION"):
                return False
            if not self.run_stage_script("security_scan.py", "SECURITY SCAN"):
                return False

            self.log(
                "BUILD", f"Building Docker Image medagent-v{self.version}...", "INFO"
            )
            time.sleep(1)
            ...
            e2e_result = subprocess.run(
                [self.python_exe, "tests/pre_launch_check.py"], cwd=self.root_dir
            )
            ...
            self.log(
                "DEPLOY", "Switching traffic to GREEN production version...", "PASS"
            )
    ...
    def update_readme_metadata(self):
        ...
        if os.path.exists(readme_path):
            ...
            if "**Last Validated:**" in content:
                content = re.sub(
                    r"\*\*Last Validated:\*\*.*", f"**Last Validated:** {now}", content
                )
            else:
                # Insert before License or at end
                if "## 📜 License" in content:
                    content = content.replace(
                        "## 📜 License", f"**Last Validated:** {now}\n\n## 📜 License"
                    )
                else:
                    content += f"\n\n---\n**Last Validated:** {now}"

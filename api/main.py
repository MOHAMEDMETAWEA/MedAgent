...
from api.routes import (auth, clinical, docs, ehr, feedback, governance,
                        imaging, learning, patient, system)
...
async def register(req: RegisterRequest):
    ...
    if (
        pers.get_user_by_login(req.username)
        or pers.get_user_by_login(req.email)
        or pers.get_user_by_login(req.phone)
    ):
        raise HTTPException(
            status_code=400,
            detail="Account with this username/email/phone already exists",
        )
...
async def set_interaction_mode(
    req: ModeRequest, user: dict = Depends(get_current_user)
):
    pers = get_persistence()
...
async def verify_doctor(
    ...
    success, message = agent.verify_doctor_credentials(
        user["sub"], req.license_number, req.specialization, country
    )
    if not success:
...
async def login(req: LoginRequest, request: Request):
    ...
    result, error = auth_agent.validate_login(
...
async def export_fhir(req: InteropRequest, user: dict = Depends(get_current_user)):
    pers = get_persistence()
    gov = get_governance()
    from database.models import Interaction, MedicalReport
        ...
        data.append(
            {
                "type": "Medication",
                "date": "Active",
                "content": f"{m['name']} {m['dosage']} {m['frequency']}",
            }
        )
...
@app.post("/consult")
async def consult(
    request: PatientRequest, background_tasks: BackgroundTasks, req: Request = None
):
    try:
        # Rate limiting
        if req:
        ...
        if result.get("critical_alert"):
            # TODO: Implement actual emergency notification task
            pass
...
async def labs_interpret(
    req: LabInterpretRequest, user: dict = Depends(get_current_user)
):
...
async def export_report(user: dict = Depends(get_current_user)):
    ...
    db = pers._get_db()
    try:
        from database.models import Interaction, PatientProfile
...
async def health_check():
    ...
    return {
        ...
        "performance": {"startup_mode": "asynchronous_optimized"},
    }
...
async def pediatric_explain(
    clinical_finding: str, age: int = 8, token: str = Depends(oauth2_scheme)
):

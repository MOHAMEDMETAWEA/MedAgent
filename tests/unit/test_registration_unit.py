...
def test_duplicate_registration():
    ...
    uid1 = pers.register_user(
        username=unique, email=f"{unique}@example.com", phone=f"+1{unique}",
        password="Pass1!", full_name="Dup Test", role="patient"
    )
    assert uid1 is not None, "First registration should succeed"
    
    uid2 = pers.register_user(
        username=unique, email=f"{unique}_2@example.com", phone=f"+2{unique}",
        password="Pass2!", full_name="Dup Test 2", role="patient"
    )
...
def test_multi_user_login():
    ...
    for i in range(3):
        ...
        uid = auth.persistence.register_user(
            username=un, email=em, phone=f"+3{i}{pid}",
            password=pw, full_name=f"Login User {i}", role="patient"
        )
...
def test_session_isolation():
    ...
    for i in range(2):
        ...
        auth.persistence.register_user(
            username=un, email=f"s_{i}_{pid}@ex.com", phone=f"+4{i}{pid}",
            password=f"SessPass{i}!", full_name=f"Sess User {i}", role="patient"
        )
        result, _ = auth.validate_login(un, f"SessPass{i}!", ip="127.0.0.1")
        sessions.append(result)
    
    assert sessions[0]["session_id"] != sessions[1]["session_id"], "Session IDs should differ"

...
def test_adaptive_communication():
    ...
    p_resp = requests.post(
        ...
        json={"login_id": patient_username, "password": "Pass123!"},
    )
    ...
    d_resp = requests.post(
        f"{API_BASE}/auth/login",
        json={"login_id": doctor_username, "password": "Pass123!"},
    )
    ...
    if r_p.ok:
        ...
        if (
            "doctor" in output.lower()
            or "care" in output.lower()
            or "emergency" in output.lower()
        ):
    ...
    if r_d.ok:
        ...
        if (
            "high blood pressure" not in output.lower()
            or "hypertension" in output.lower()
        ):

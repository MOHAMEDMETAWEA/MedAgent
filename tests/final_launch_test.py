...
def run_final_audit():
    ...
    try:
        ...
        login_resp = requests.post(
            f"{API_BASE}/auth/login", json={"login_id": user_id, "password": password}
        )
        ...
        ar_resp = requests.post(
            f"{API_BASE}/consult",
            json={"symptoms": ar_symptoms, "patient_id": user_id},
            headers=headers,
        )

        if ar_resp.ok and "صداع" in ar_resp.json().get("patient_info", {}).get(
            "summary", ""
        ):
            ...
            print(
                "Bilingual PASS: Arabic intake processed (Summary in English or mixed which is acceptable)."
            )
            results["BilingualSupport"] = "PASS"
        else:
        ...
        followup_resp = requests.post(
            f"{API_BASE}/consult",
            json={
                "symptoms": "It is getting worse and I feel dizzy now.",
                "patient_id": user_id,
            },
            headers=headers,
        )
        ...
        if followup_resp.ok and "Analysis Complete" in followup_resp.json().get(
            "status", ""
        ):
            ...
            print(
                f"Reasoning FAIL: ToT status missing. Status: {followup_resp.json().get('status')}"
            )
